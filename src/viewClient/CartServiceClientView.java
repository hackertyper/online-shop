...
public class CartServiceClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private CartServiceView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public CartServiceClientView( ExceptionAndEventHandler parent, CartServiceView service) {
		super();
		this.parent = parent;
	...
	private CartServiceView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getCartServiceView();
		if (object == null){
	...
	public CartServiceConnection getConnection(){
        	return (CartServiceConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getCartServiceView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getCartServiceView());	
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().cartService_Menu_Filter((Anything)selected));
            } catch (ModelException me){
