...
public class ShopServiceClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private ShopServiceView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public ShopServiceClientView( ExceptionAndEventHandler parent, ShopServiceView service) {
		super();
		this.parent = parent;
	...
	private ShopServiceView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getShopServiceView();
		if (object == null){
	...
	public ShopServiceConnection getConnection(){
        	return (ShopServiceConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getShopServiceView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getShopServiceView());	
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().shopService_Menu_Filter((Anything)selected));
            ...
            if (selected instanceof ArticleView){
                ...
                item.setOnAction(new EventHandler<ActionEvent>(){
                    public void handle(javafx.event.ActionEvent e) {
                        final ShopServiceAddToCartArticleIntegerMssgWizard wizard = new ShopServiceAddToCartArticleIntegerMssgWizard("Zum Einkaufswagen hinzufügen");
	...
	class ShopServiceAddToCartArticleIntegerMssgWizard extends Wizard {

		protected ShopServiceAddToCartArticleIntegerMssgWizard(String operationName){
		...
		protected void initialize(){
			this.helpFileName = "ShopServiceAddToCartArticleIntegerMssgWizard.help";
			super.initialize();		
		}
	...
	class ShopServiceFindArticleStringMssgWizard extends Wizard {

		protected ShopServiceFindArticleStringMssgWizard(String operationName){
			super(ShopServiceClientView.this);
			getOkButton().setText(operationName);
			getOkButton().setGraphic(new FindArticlePRMTRStringPRMTRMenuItem ().getGraphic());
		}
		protected void initialize(){
			this.helpFileName = "ShopServiceFindArticleStringMssgWizard.help";
			super.initialize();		
		}
	...
	private ImageView getIconForMenuItem(ShopServiceMenuItem menuItem){
