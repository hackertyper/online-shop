...
public class OrderServiceClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private OrderServiceView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public OrderServiceClientView( ExceptionAndEventHandler parent, OrderServiceView service) {
		super();
		this.parent = parent;
	...
	private OrderServiceView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getOrderServiceView();
		if (object == null){
	...
	public OrderServiceConnection getConnection(){
        	return (OrderServiceConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getOrderServiceView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getOrderServiceView());	
    ...
    private class SignalChangedPRMTRMenuItem extends OrderServiceMenuItem{
        protected ImageView accept(MenuItemVisitor visitor){
            return visitor.handle(this);
        }
    }
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().orderService_Menu_Filter((Anything)selected));
            } catch (ModelException me){
	...
	private ImageView getIconForMenuItem(OrderServiceMenuItem menuItem){
