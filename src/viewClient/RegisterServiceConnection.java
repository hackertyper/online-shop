...
public class RegisterServiceConnection extends ServiceConnection {

	public RegisterServiceConnection (String url, ExceptionAndEventHandler handler, ClientReporter reporter) throws MalformedURLException {
		super(url, handler, reporter);
	}

	public RegisterServiceView getRegisterServiceView(){
		return (RegisterServiceView)super.getServer();
	}

    public synchronized String registerService_Menu_Filter(Anything anything) throws ModelException{
        try {
            ...
            java.util.HashMap<?,?> success = (java.util.HashMap<?,?>)this.execute(this.connectionName, "registerService_Menu_Filter", parameters);
    ...
    public synchronized void register(String accountName, String password) throws ModelException, DoubleUsername{
        try {
            ...
            if(!((Boolean)success.get(common.RPCConstantsAndServices.OKOrNotOKResultFieldName)).booleanValue()){
                if (((Integer)success.get(common.RPCConstantsAndServices.ErrorNumberFieldName)).intValue() == 0)
                    throw new ModelException((String)success.get(common.RPCConstantsAndServices.ExceptionMessageFieldName), ((Integer)success.get(common.RPCConstantsAndServices.ExceptionNumberFieldName)).intValue());
                if(((Integer)success.get(common.RPCConstantsAndServices.ErrorNumberFieldName)).intValue() == -181)
